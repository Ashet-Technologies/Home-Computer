...
bar_slider_size = 8;              // mm, what's the square edge width?
...
backplane_height = slot_height * slot_count; // mm, height of backplace PCB

// Case Configuration:
panel_thick = 1.6; // mm, panel material thickness
strut_edge = 8;    // mm, what's the square edge width of a structural strut?
...
case_length = backplane_pos + strut_edge/2; // pcb_length + backplane_padding;
...
module pcb_guide()
{
    ...
    {
        cube([ bar_slider_size, length, bar_slider_size ]);
...
module vstrut()
{
    mat_brass() difference()
    {
        ...
        // vertical backplane cut
        translate([
                bar_slider_size - bar_slider_inset, (strut_edge-bar_slider_sep)/2, -bar_slider_sep
            ]) cube([ strut_edge, bar_slider_sep, case_height + 2 * bar_slider_sep ]);
    }
}
...
module hstrut()
{
    mat_brass() cube([ strut_width, strut_edge, strut_edge ]);
}
...
module sidepanel()
{
    mat_pcb() cube([ panel_thick, case_length, case_height ]);
}
...
module toppanel()
{
    mat_pcb() cube([ case_width + 2 * panel_thick, case_length, panel_thick ]);
}
...
module backpanel()
{
    mat_pcb() cube([ case_width + 2 * panel_thick, panel_thick, case_height + 2 * panel_thick ]);
}
...
module backplane()
{
    ...
    {
        mat_pcb() cube([ backplane_width, pcb_thick, backplane_height ]);
