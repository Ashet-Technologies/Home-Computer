...
function get_coarse_pitch(dia) = lookup(dia, [
    ...
    [ 10, 1.5 ], [ 12, 1.75 ],  [ 14, 2 ],    [ 16, 2 ],     [ 18, 2.5 ],   [ 20, 2.5 ], [ 22, 2.5 ],
    [ 24, 3 ],   [ 27, 3 ],     [ 30, 3.5 ],  [ 33, 3.5 ],   [ 36, 4 ],     [ 39, 4 ],   [ 42, 4.5 ],
    [ 45, 4.5 ], [ 48, 5 ],     [ 52, 5 ],    [ 56, 5.5 ],   [ 60, 5.5 ],   [ 64, 6 ],   [ 78, 5 ]
]);
...
function rolson_hex_nut_dia(dia) = lookup(dia, [
    [ 3, 6.4 ], [ 4, 8.1 ], [ 5, 9.2 ], [ 6, 11.5 ], [ 8, 16.0 ], [ 10, 19.6 ], [ 12, 22.1 ], [ 16, 27.7 ],
    [ 20, 34.6 ], [ 24, 41.6 ], [ 30, 53.1 ], [ 36, 63.5 ]
]);
// function for Rolson hex nut height from thread size
function rolson_hex_nut_hi(dia) = lookup(dia, [
    [ 3, 2.4 ], [ 4, 3.2 ], [ 5, 4 ], [ 6, 3 ], [ 8, 5 ], [ 10, 5 ], [ 12, 10 ], [ 16, 13 ], [ 20, 16 ], [ 24, 19 ],
    [ 30, 24 ], [ 36, 29 ]
]);

// function for Fairbury hex nut diameter from thread size
function fairbury_hex_nut_dia(dia) = lookup(dia, [
    [ 3, 6.0 ], [ 4, 7.7 ], [ 5, 8.8 ], [ 6, 11.0 ], [ 8, 14.4 ], [ 10, 17.8 ], [ 12, 20.0 ], [ 16, 26.8 ],
    [ 20, 33.0 ], [ 24, 40.0 ], [ 30, 50.9 ], [ 36, 60.8 ]
]);
// function for Fairbury hex nut height from thread size
function fairbury_hex_nut_hi(dia) = lookup(dia, [
    [ 3, 2.2 ], [ 4, 3.0 ], [ 5, 4.5 ], [ 6, 5.0 ], [ 8, 6.5 ], [ 10, 8.1 ], [ 12, 10.4 ], [ 16, 14.2 ], [ 20, 17 ],
    [ 24, 20.3 ], [ 30, 24.4 ], [ 36, 29.5 ]
]);
...
//  diaThreadIn=diameter of the thread inside
{
    ...
    {
        // We need to know the height of the bolt head so we can offset the threaded hole because the bolt head is at
        // the bottom, near Z=0.
        ...
        {
            if (WrenchSizes == 0)
                rolson_hex_bolt(dia, hi);
            else if (WrenchSizes == 1)
                fairbury_hex_bolt(dia, hi);
            else
                small_hex_bolt(dia, hi);
    ...
    {
        echo("The diameter for the thread inside (", diaThreadIn,
             ") + 1 cannot be equal or higher than the diameter of the bolt (", dia, ")!");
    }
}
...
//  sh = segment height (z)
{
    ...
    polyhedron(points = [[cos(as) * rt, sin(as) * rt, z],                                             // 0
...
//  sh = segment height (z)
{
    ...
    //  view from front (x & z) extruded in y by sg
    //
    polyhedron(points = [[cos(as) * (rt + (5 * h)), sin(as) * (rt + (5 * h)), z],                     // 0
